...
import { createHeroMiniAbilityInteractives } from './HeroMiniAbilityAssemblage'

const HeroMiniAbilityPopupAssemblage = (
  card: RelaxedCardInstance,
  owner: Player,
  shouldGlow?: boolean
) => {
  const { visualsRoot, highlight } = createHeroMiniAbilityInteractives(
    card,
    owner
  )
...
export default HeroMiniAbilityPopupAssemblage
