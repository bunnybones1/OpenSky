...
export default class GlowingScreenSpaceRingGeometry extends BufferGeometry {
  constructor() {
    super()
    const gridU = 64
    const gridV = 5
    ...
    const offsetDataArr = new Float32Array(vertCount * 3)
    ...
    for (let iy = 0; iy < gridV1; iy++) {
      ...
      for (let ix = 0; ix < gridU1; ix++) {
        ...
        posArr[i2] = Math.cos(x * circleAngle)
        posArr[i2 + 1] = Math.sin(x * circleAngle)

        offsetDataArr[i3] = offset
        offsetDataArr[i3 + 1] = direction
        offsetDataArr[i3 + 2] = opacityLookup

        uvArr[i2] = Math.abs(x - 0.5) * 2
        uvArr[i2 + 1] = y
        i2 += 2
        i3 += 3
      }
    ...
    this.setAttribute(
      'offsetData',
      new Float32BufferAttribute(offsetDataArr, 3)
    )
