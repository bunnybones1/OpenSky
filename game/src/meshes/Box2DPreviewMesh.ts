...
import { rand2 } from '@opensky/shared/utils/math'
import {
  BufferGeometry,
...
import { isDecendantOf } from '~/utils/threeUtils'
import {
  Body,
  CircleShape,
  Fixture,
...
function getPolygonShapeWorldVerts(polygon: PolygonShape, body: Body) {
  return polygon.m_vertices.map(vert => {
    ...
    worldVert.x /= renderMetrics.aspect
    return worldVert
  })
}
...
export class Box2DPreviewMesh extends Line {
  ...
  constructor(myB2World: World) {
    ...
    super(createGeometry(myB2World, debugColors), new Box2DPreviewMaterial())
    this.debugColors = debugColors
    this.myB2World = myB2World

    this.renderOrder = RENDER_ORDERS.debug
    this.addEventListener('added', this.onAdd)
  }
  onAdd() {
