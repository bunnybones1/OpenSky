...
export default class EndMatchContinueButtonContainer extends UIContainer {
  ...
  constructor(ui: UI, priority: number) {
    super(ui, 'endMatchContinueButton', {
      priority
    })
  }
  ...
  protected init() {
    ...
    this.continue = new Promise(resolve => {
      const continueButton = createButton(
        ...
        () => {
          ...
          if (this.ui.hasContainer('actionHistory')) {
            ...
            this.ui.getContainer('hud').buttonActionHistory.mesh.visible = false
          ...
          if (this.ui.hasContainer('deckSidebars')) {
            const deckSideBars = this.ui.getContainer('deckSidebars')
            if (deckSideBars.initd) {
              deckSideBars.playerDeckSidebar.closeAndLock()
              deckSideBars.playerGraveyardSidebar.closeAndLock()
              deckSideBars.opponentGraveyardSidebar.closeAndLock()
            }
          }
      ...
      // this.button.mesh.scale.setScalar(device.isMobile ? 0.9 : 1.5)
      giveButtonKeyboardShortcut(this.button, ' ')
      if (queryParams.skipMatchEndContinue) {
        resolve()
      }
    })
  }
