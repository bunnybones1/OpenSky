...
interface ScrollViewOptions<T> {
  ...
  scissor?: boolean
  inverted?: boolean
  ommitCollider?: boolean
}

type HoverListener = (item: Object2D) => void

export default class ScrollView<T extends Object2D> extends Object2D {
  ...
  private _itemSorter: Sorter<T> | undefined
  get itemSorter(): Sorter<T> | undefined {
    return this._itemSorter
  }
  set itemSorter(value: Sorter<T> | undefined) {
    this._itemSorter = value
    this.makeDirty()
  }
  innerContainer: Object2D = new Object2D()
  items: T[] = []
  itemOffsets: Map<T, number | undefined> = new Map()
  ...
  constructor(options: ScrollViewOptions<T>) {
    super()

    this.name = 'scrollview'
    if (!options.ommitCollider) {
  ...
  private _updateAfterAdd(item: T) {
  ...
  recalculate = (iOffset = 0) => {
    if (this._itemSorter) {
      this.items.sort(this._itemSorter)
    }
    // Reposition items
