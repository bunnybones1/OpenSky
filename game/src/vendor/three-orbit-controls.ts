 ...
 * @author arilotter / https://arilotter.com/
 *
  ...
  constructor(object: Camera, domElement?: HTMLElement, domWindow?: Window) {
    ...
    this.onTouchMove = (event: ThreeEvent) => {
      ...
      switch (event.touches.length) {
        ...
        case 2:
          {
            if (!this.enableZoom || this.state !== STATE.TOUCH_DOLLY) {
              return
            }
        ...
        case 3:
          {
            if (!this.enablePan || this.state !== STATE.TOUCH_PAN) {
              return
            }
  ...
  pan(deltaX: number, deltaY: number) {
    ...
    if (this._checkPerspectiveCamera(this.object)) {
      ...
      const targetDistance =
        this.panInternalOffset.length() *
        Math.tan(((this.object.fov / 2) * Math.PI) / 180.0)
