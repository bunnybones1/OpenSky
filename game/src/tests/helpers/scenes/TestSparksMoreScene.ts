...
class TestSparksMoreScene extends BaseTestScene {
  private _fountainEmitters: Emitter[] = []
  private _sparks: QuadraticPoints
  ...
  async init() {
    ...
    await getAssetsManager().loadAsset('noise3Map')
    this.regenerateSparks(60000)
  }
  ...
  regenerateSparks(total: number) {
    ...
    const sparks = new QuadraticPoints({
      speed: 0.2,
      matOptions: {
    ...
    const cursorEmitter = sparks.geometry.createEmitter(
      new Vector3(),
      missileParticlePathMakerLib.sparks
    )
    ...
    const fountainEmitters = [
      {
        ...
        ssy: 0.5,
        anglez: 0,
        ppm: missileParticlePathMakerLib.sparkField
      },
      {
        ssx: 0.5,
        ssy: 0.5,
        anglez: 0,
        ppm: missileParticlePathMakerLib.sparkSinkhole
      }
    ].map(props => {
      console.log(
    ...
    if (this._bufferRenderBar) {
      this._bufferRenderBar.max = sparks.geometry.total
    }
  ...
  async initUI(ui: UI) {
    ...
    buttonDatas.push(
      new QuickButtonData('double buffer', () => {
        this.regenerateSparks(this._sparks.geometry.total * 2)
      })
    )
    buttonDatas.push(
      new QuickButtonData('half buffer', () => {
        this.regenerateSparks(this._sparks.geometry.total * 0.5)
      })
    )
  ...
  update(dt: number) {
    ...
    if (this._sparks) {
      for (const fountain of this._fountainEmitters) {
        fountain.update(fountain.position, fountain.quaternion, 0, 6000)
      }
...
export const scene = TestSparksMoreScene
