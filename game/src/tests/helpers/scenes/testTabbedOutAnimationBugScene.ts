...
async function testTabbedOutAnimationBugScene() {
  ...
  const ids: BaseCard[] = ['2005', '2008', '7', '8', '1023', '4099', '2099']
  ...
  function cardIDToEnt(id: BaseCard) {
    ...
    ent.get('zone')!.setStateZone('Deck')
  ...
  for (const ent of ents) {
    const zone = ent.get('zone')
    zone.setStateZone('CardSelection')
    zone.setUserZone('UseState')
  }

  await animationDelay(1000)
  for (const index of [0, 1, 11, 12]) {
  ...
  for (const index of [0, 1, 11, 12]) {
    ...
    if (ent.has('isAnimating')) {
      ...
      ent.getComponent('isAnimating')!.onComponentDetach.then(() => {
...
export const test = testTabbedOutAnimationBugScene
