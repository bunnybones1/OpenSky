...
class TestMagicRibbonsScene extends BaseTestScene {
  ...
  update(dt: number) {
    if (this._sparks) {
      ...
      for (const fountain of this._fountainEmitters) {
        ...
        fountain.update(fountain.position, fountain.quaternion, 0, 2000)
      }
    }
    super.update(dt)
  }
...
export const scene = TestMagicRibbonsScene
