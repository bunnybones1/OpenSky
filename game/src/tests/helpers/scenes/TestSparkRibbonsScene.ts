...
class TestSparkRibbonsScene extends BaseTestScene {
  private _fountainEmitters: Emitter[] = []
  private _sparks: QuadraticRibbonsMesh
  ...
  onDragMoveEmitter = (x: number, y: number) => {
    ...
    this._cursorEmitter.update(pos, this._cursorEmitter.quaternion, 1000, 0)
  }
  ...
  regenerateSparks(total: number) {
    ...
    const trianglesPerRibbon = 4
    const sparks = new QuadraticRibbonsMesh({
      total,
      speed: 2,
      trianglesPerRibbon,
      matOptions: {
        strokePathFraction: 0.06,
        relativeWidth: 0.002,
    ...
    const cursorEmitter = sparks.geometry.createEmitter(
      new Vector3(),
      missileParticlePathMakerLib.sparks2
    )
    ...
    const fountainEmitters = [
      {
        ...
        ssy: 0.4,
        anglez: 0.4,
        ppm: missileParticlePathMakerLib.sparks
      },
      {
        ssx: 0.4,
        ssy: 0.5,
        anglez: 0.2,
        ppm: missileParticlePathMakerLib.sparks2
      },
      {
        ssx: 0.5,
        ssy: 0.6,
  ...
  update(dt: number) {
    if (this._sparks) {
      ...
      for (const fountain of this._fountainEmitters) {
        fountain.rotation.y += dt * 0.6
        fountain.update(fountain.position, fountain.quaternion, 0, 200)
      }
    }
    super.update(dt)
  }
...
export const scene = TestSparkRibbonsScene
