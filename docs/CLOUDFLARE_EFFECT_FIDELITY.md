...
| Main Worker's one-minute fan-out                                                                                  | Redesign incrementally      | Conquest V2 and leaderboard cron work only discovers/recovers deterministic Workflows, while delayed Gold and external push only re-drive due D1 receipts to Queues; the remaining unrelated responsibilities still need individual target boundaries. |
...
| Delayed Conquest Gold Queue handoff in migration `0123`                                                           | Keep undeployed             | `e4ec21f5` preserves exact 24-hour delivery, moderation, atomic inventory/feed publication, and re-drive while removing direct cron grants and terminal attempt state.                                                                                 |
...
page/attempt limits. Delayed Conquest Gold completed locally at `e4ec21f5`:
one narrow delayed Queue message references each D1 entitlement, the consumer
applies the due off-chain effect atomically, and cron only re-drives due
