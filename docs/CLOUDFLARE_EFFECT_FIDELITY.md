...
| Main Worker's one-minute fan-out                                                                                  | Redesign incrementally      | Conquest V2 and leaderboard cron work only discovers/recovers deterministic Workflows, while delayed Gold only re-drives due D1 receipts to a Queue; the remaining unrelated responsibilities still need individual target boundaries. |
