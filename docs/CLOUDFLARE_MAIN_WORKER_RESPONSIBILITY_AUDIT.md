...
| `runReferralStickerRewards`             | Carry referral progress, freeze delayed sticker awards, and deliver off-chain inventory            | Workflow per accepted hourly sweep plus Queue per prepared user or due award batch      | Selected in `CLOUDFLARE_REFERRAL_STICKER_ORCHESTRATION.md`; implementation next  |
