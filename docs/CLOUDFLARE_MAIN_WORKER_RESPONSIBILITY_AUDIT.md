...
| `dispatchPendingConquestReadinessDrills` | Recover the Workflow for an explicitly authorized drill without advancing D1 state                 | One deterministic Workflow per D1 operation; Cron only re-ensures incomplete instances | Implemented and guarded locally through migration `0128`; undeployed |
