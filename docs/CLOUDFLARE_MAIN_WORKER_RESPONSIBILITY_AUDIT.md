...
| `runConquestReadinessDrills`           | Advance an explicitly authorized operational drill and preserve its audit trail                    | One deterministic Workflow per D1 operation; Cron may only re-ensure incomplete instances | Boundary selected; direct Cron runner still present pending implementation |
