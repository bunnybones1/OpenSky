    ...
    run: |game, queue, my_id, phase, _| {
      Box::pin(async move {
        ...
        }) = phase.try_into()
        {
          ...
          if game.owner(id) == owner && id != my_id {
            queue.add_resolution(move |game| {
              Box::pin(async move {
                let enemy_units = game.units(enemy(owner));
                let dead_ally_pwr = game.reveal_from_card(dead_card, |c| c.power).await;
                game
                  .smart_random_damage(enemy_units, dead_ally_pwr.into(), my_id)
                  .await;
              })
            });
          }
