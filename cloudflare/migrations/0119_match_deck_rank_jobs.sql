-- The source API does not mutate deck aggregates in InternalMatchEnd. It
-- enqueues a DeckRankUpdateTask in the same transaction as the terminal match
-- row, then a separate worker reads that committed match and applies the deck
-- changes. Stage the same durable responsibility before the Workers
-- publication barrier, but permit its effects only after the ledger is ended.
...
-- A pending job may start exactly one due attempt, become terminal after its
-- fifth failed attempt, or become applied only through the exact receipt
-- installed by the aggregate batch below. All source task identity fields are
-- immutable.
