...
- Paused branch checkpoint: runtime hardening commit `339d7f9d` is tested but not
  ...
  exposed.
  Public recent-match recovery also preserves the Go
  `[2]proto.Conquest` contract. The game server may omit `conquestInfo` for a
  non-Conquest stored result, but the gateway emits the two complete Go
  zero/default objects that `/matchinfo` originally produced. Real Conquest
  pairs are re-serialized, a missing or malformed required pair fails closed,
  and participant-only recent store/reward access remains unchanged.
  Mixed modes preserve the source's separate public and completed projections:
  pending and active `MatchInfo.mode` uses player one's requested mode for both
  participants, while replay initialization and completed recent-match state
  use the match-wide equal mode or `UNKNOWN` for a mixed Practice PvP/ranked
  pair. Per-participant allocation modes remain authoritative internally.
  Public `GameServerInfo.status` also uses the source registry's `running`
  value for an active or initializing per-proposal server; the invented
  Cloudflare-only `online` value is not part of the matchmaker wire.
  Matchmaker errors likewise preserve Go `NewErrorMessage`: `reason` and
...
- Source-faithful Conquest V2 weekly treasure rollover and delayed off-chain
  ...
  started cycles remain deliverable from immutable D1 receipts through one
  deterministic Workflow per cycle and one Queue message per player
  entitlement. Queue retry/DLQ state is transport evidence, never terminal
  business state
...
- Source `PromoteGrandmastersRunner` effect through a separately serialized
  global Durable Object responsibility: terminal clients never wait for an
  attempt, D1 retains a `PENDING` responsibility until atomic application, and
  target-owned bounded exponential retry remains recoverable without a copied
