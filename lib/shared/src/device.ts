...
class Device {
  ...
  handleChange = () => {
    ...
    const targetWidth = this._overrideWidth === undefined ? window.innerWidth : this._overrideWidth
    const targetHeight = this._overrideHeight === undefined ? window.innerHeight : this._overrideHeight
  ...
  get supportsModulesInWorker() {
    let supports = false;
    const tester = {
      get type() { supports = true; return 'classic' as const} // it's been called, it's supported
    };
    try {
  ...
  get performance() {
    if (this.fps < this.targetFPS * 0.4) {
      // < 24 FPS
      return 'low'
    } else if (this.fps < this.targetFPS * 0.8) {
      // > 24 < 48 FPS
      return 'medium'
    } else {
      // > 48 FPS
