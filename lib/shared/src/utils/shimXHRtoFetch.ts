// @ts-nocheck
// a modified version of this sneaky shim https://gist.github.com/yigaldviri/f77066e7fb596ec7d5e4dce6b7a4ddf1
import { cacheyFetch } from "./cacheyFetch"
...
export class FetchingXMLHttpRequest extends Dispatch {
    ...
    fetch(this.url, {
      ...
      .then(response => {
        ...
        switch (this.responseType) {
          ...
          case 'cors':
            if (this.url.endsWith('.gltf') || this.url.endsWith('.json')) {
              return response.text()
              // return response.json()
            } else {
              return response.arrayBuffer()
  ...
  get responseText() {
    ...
    throw new Error('no responseText')
  }
