...
export function audioLevelCurve(v: number) {
  return Math.pow(v, 2)
}

export function groupSizes(maxGroupSize: number, cards: number): number[] {
  const numGroups = Math.ceil(cards / maxGroupSize)
  const cardsPerGroup = Math.floor(cards / numGroups)
  const numExtraCards = cards - cardsPerGroup * numGroups
  return Array.from({ length: numGroups }, (_, i) =>
    i < numExtraCards ? cardsPerGroup + 1 : cardsPerGroup
  )
}
